...
import { 
  ...
  BookOpen, Flame, Target, Award, BarChart3
} from 'lucide-react';
...
interface MonthData {
  ...
  totalAyahs: number;
  activeDays: number;
  weeklyBreakdown: { week: number; pages: number; ayahs: number }[];
}
...
export function AnalyticsDashboard() {
  ...
  const fetchWeeklyData = async () => {
    ...
    try {
      ...
      const dailyData: DayData[] = days.map(day => {
        ...
        const dayReading = readingData?.filter(r => 
          r.created_at.startsWith(dateStr)
        ) || [];
        const dayMem = memData?.filter(m => 
          m.updated_at.startsWith(dateStr)
        ) || [];
  ...
  const fetchMonthlyData = async () => {
    ...
    try {
      ...
      const weeklyBreakdown: { week: number; pages: number; ayahs: number }[] = [];
      ...
      for (let week = 0; week < 5; week++) {
        ...
        weeklyBreakdown.push({ week: week + 1, pages: weekPages, ayahs: weekAyahs });
      }
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <span className="font-medium text-foreground">
          {view === 'weekly' 
            ? `${new Date(weekData?.weekStart || '').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} - ${new Date(weekData?.weekEnd || '').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`
                  ...
                  <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                    <div 
                      className={cn(
                        ...
                        day.pagesRead > 0 ? "bg-primary" : "bg-muted"
                      )}
                      style={{ 
              ...
              <div className="flex gap-1">
                {weekData.dailyData.map((day) => (
                  <div
                    ...
                    title={`${new Date(day.date).toLocaleDateString()}: ${day.pagesRead} pages`}
                  />
                ))}
              </div>
            </div>
                ...
                {monthData.weeklyBreakdown.map((week) => (
                  <div key={week.week} className="flex-1 flex flex-col items-center gap-1">
                    <div 
                      className={cn(
                        "w-full rounded-t-md transition-all",
                        week.pages > 0 ? "bg-primary" : "bg-muted"
                      )}
                      style={{ 
